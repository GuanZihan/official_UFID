...
def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], num_inference_steps: int=1000, ddim_eta: float=None, init: torch.Tensor=None, max_batch_n: int=256, rng: torch.Generator=None):
    ...
    for i, batch_sz in enumerate(batch_sizes):
        ...
        if i > 0:
            break
        if ddim_eta == None:
            pipline_res = pipeline(num_inference_steps=num_inference_steps, batch_size=batch_sz,  generator=rng, init=init[i], output_type=None)
        else:
            pipline_res = pipeline(num_inference_steps=num_inference_steps, batch_size=batch_sz,  generator=rng, init=init[i], output_type=None, eta=ddim_eta)
        # sample_imgs_ls.append(pipline_res.images)
