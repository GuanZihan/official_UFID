...
class CondLossFn:
    ...
    def get_loss_by_keys(self, batch, unet, latent_key: str, timesteps: torch.Tensor=None, weight: float=1.0, poison_latent_key: str=None, caption_key: str=None, noise: torch.Tensor=None, backdoor: bool=False):
        ...
        # print(res[2].shape) # (1, 77, 768)

        # ======
        # augmented_noise = torch.cat([res[0], res[0]])
        # none_caption = [''] * res[0].shape[0]
        ...
        # none_input_ids = tokenizer(none_caption,
        #     padding="max_length",
        #     max_length=res[2].shape[1],
        #     truncation=True,
        #     return_tensors="pt",
        #     )

        # attention_mask = none_input_ids.attention_mask.to(res[0].device)     
        # none_caption_embedding = self.__text_encoder(
        #     none_input_ids.input_ids.to(res[0].device),
        #     attention_mask=attention_mask,
        # )[0]

        # preds_none_caption = unet(res[0], timesteps, none_caption_embedding).sample

        # print(torch.norm(preds_none_caption - unet(res[0], timesteps, res[2]).sample))
        # print(timesteps)
        # input()
        # ======
