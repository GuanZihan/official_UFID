...
from caption_dataset import CaptionBackdoor, Backdoor, DatasetLoader, ImagePathDataset, ReplicateDataset
...
class Sampling:
    ...
    def _batch_sampling(prompts: List[str], pipeline: DiffusionPipeline, inits: torch.Tensor=None,
        ...
        with torch.no_grad():
            ...
            for prompt_batch, init_batch in zip(prompt_batchs, init_batchs):
                ...
                # print(init_batch, handle_batch_fn)
                # input()
                batch_imgs = pipeline_call(prompt=prompt_batch, latents=init_batch)
                batch_imgs = batch_imgs.images
    ...
    def augment_prompts(prompts: Union[str, List[str]], img_num_per_prompt: int):
        ...
        if isinstance(prompts, str): 
            ...
            for idx in range(len(prompts)):
                if idx == 0:
                    continue
                prompts[idx] = random.choice(pre_detections) + prompts[idx]
        elif isinstance(prompts, list):
            prompts: List[str] = prompts * img_num_per_prompt
        else:
            raise TypeError(f"Arguement prompts should be a list of strings or string, not {type(prompts)}")
    ...
    def backdoor_clean_samples(self, pipe: DiffusionPipeline, prompts: str, image_trigger: str=None, caption_trigger: str=None,
        ...
        if caption_trigger is not None:
            images = self.caption_backdoor_sample(prompts=prompts, trigger=caption_trigger, pipe=pipe, start_pos=trig_start_pos, end_pos=trig_end_pos, inits=None, handle_fn=handle_fn, handle_batch_fn=handle_batch_fn, seed=seed, return_imgs=return_imgs)
        elif image_trigger is not None:
            images = self.image_backdoor_sample(prompts=prompts, trigger=image_trigger, pipe=pipe, inits=None, handle_fn=handle_fn, handle_batch_fn=handle_batch_fn, seed=seed, return_imgs=return_imgs)
        else:
            images = self.clean_sample(prompts=prompts, pipe=pipe, inits=None, handle_fn=handle_fn, handle_batch_fn=handle_batch_fn, seed=seed, return_imgs=return_imgs)
        return images
    ...
    def generate_samples(self, base_path: Union[os.PathLike, str], pipe: DiffusionPipeline, prompts: Union[List[str], str], image_trigger: str=None, caption_trigger: str=None,
        ...
        if isinstance(prompts, str):
            return self.generate_sample(base_path=base_path, pipe=pipe, prompt=prompts, image_trigger=image_trigger, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, img_num_per_grid_sample=img_num_per_grid_sample, _format=_format, seed=seed, force_regenerate=force_regenerate)
        elif isinstance(prompts, list):
            all_detection_results = []
            for prompt in prompts:
                r = self.generate_sample(base_path=base_path, pipe=pipe, prompt=prompt, image_trigger=image_trigger, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, img_num_per_grid_sample=img_num_per_grid_sample, _format=_format, seed=seed, force_regenerate=force_regenerate)
                all_detection_results.append(r)
...
class PromptDataset:
    ...
    def __get_datasetloader(ds_root: str, ds_name: str, split: str) -> 'DatasetLoader':
        return DatasetLoader(root=ds_root, name=ds_name, batch_size=128, split=split, num_workers=0, force_R_to_0=True).set_poison(trigger_type=Backdoor.TRIGGER_NONE, caption_trigger_type=CaptionBackdoor.TRIGGER_NONE, rand_caption_trig_pos=0, target_type=Backdoor.TARGET_TG, clean_rate=1.0, poison_rate=0.0).prepare_dataset(mode=DatasetLoader.MODE_FIXED)
...
class Measuring():
    ...
    def compute_fid_score(self, clean_path: Union[str, os.PathLike], backdoor_path: Union[str, os.PathLike], max_batch_n: int=MeasuringStatic.FID_MAX_BATCH_N):
        ...
        return float(fid(path=[clean_path, backdoor_path], device=self.__device, num_workers=0, batch_size=max_batch_n))
    ...
    def sample_by_part(self, pipe: DiffusionPipeline, store_path: Union[str, os.PathLike], in_out_dist: str, train_test_split: str,
        ...
        # print(prompts)
        # input()
        return self.__sampling.generate_samples(base_path=p, pipe=pipe, prompts=prompts, image_trigger=None, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, img_num_per_grid_sample=img_num_per_grid_sample, _format=_format, seed=seed, force_regenerate=force_regenerate)
    ...
    def AUROC_Score(self, pred_in, pred_out, file):
        y_in = [1] * len(pred_in)
    ...
    def sample(self, pipe: DiffusionPipeline, store_path: Union[str, os.PathLike], caption_trigger: str=None, 
        ...
        if caption_trigger is not None:
            # Backdoor
            backdoor_in_train = self.sample_by_part(pipe=pipe, store_path=store_path, in_out_dist=PromptDatasetStatic.IN_DIST, train_test_split=PromptDatasetStatic.TRAIN_SPLIT, prompts=None, img_num_per_grid_sample=img_num_per_grid_sample, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, _format=_format, seed=seed, force_regenerate=force_regenerate)

        np.save("backdoor_in_train", backdoor_in_train)
        np.save("clean_in_train", clean_in_train)

        self.AUROC_Score(backdoor_in_train, clean_in_train, "123")

        # In Distribution, Test
        # Clean
        clean_in_test = self.sample_by_part(pipe=pipe, store_path=store_path, in_out_dist=PromptDatasetStatic.IN_DIST, train_test_split=PromptDatasetStatic.TEST_SPLIT, prompts=None, img_num_per_grid_sample=img_num_per_grid_sample, caption_trigger=None, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, _format=_format, seed=seed, force_regenerate=force_regenerate)
        if caption_trigger is not None:
            # Backdoor
            backdoor_in_test = self.sample_by_part(pipe=pipe, store_path=store_path, in_out_dist=PromptDatasetStatic.IN_DIST, train_test_split=PromptDatasetStatic.TEST_SPLIT, prompts=None, img_num_per_grid_sample=img_num_per_grid_sample, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, _format=_format, seed=seed, force_regenerate=force_regenerate)
        self.AUROC_Score(backdoor_in_test, clean_in_test, "123")    
        np.save("backdoor_in_test", backdoor_in_test)
        np.save("clean_in_test", clean_in_test)
        # Out Distribution
        # Clean
        clean_out = self.sample_by_part(pipe=pipe, store_path=store_path, in_out_dist=PromptDatasetStatic.OUT_DIST, train_test_split=PromptDatasetStatic.FULL_SPLIT, prompts=None, img_num_per_grid_sample=img_num_per_grid_sample, caption_trigger=None, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, _format=_format, seed=seed, force_regenerate=force_regenerate)
        if caption_trigger is not None:
            # Backdoor
            backdoor_out = self.sample_by_part(pipe=pipe, store_path=store_path, in_out_dist=PromptDatasetStatic.OUT_DIST, train_test_split=PromptDatasetStatic.FULL_SPLIT, prompts=None, img_num_per_grid_sample=img_num_per_grid_sample, caption_trigger=caption_trigger, trig_start_pos=trig_start_pos, trig_end_pos=trig_end_pos, _format=_format, seed=seed, force_regenerate=force_regenerate)

        
        self.AUROC_Score(backdoor_out, clean_out, "123")
        np.save("backdoor_out", backdoor_out)
        np.save("clean_out", clean_out)
